...
struct etudiant {
    wchar_t  nom[TAILLE_CH]      ;
    wchar_t  prenom[2*TAILLE_CH] ;
    int      age                 ;
}; 
/* ----Variables globales-------- */
...
int main()
{
  ...
  {
   wprintf(L"-1-Saisie\n")         ;
   wprintf(L"-2-Affichage\n")      ;
   wprintf(L"-3-Sauvegarde\n")     ;
   wprintf(L"-4-Chargement\n")     ;
   wprintf(L"-5-Vider la liste\n") ;
   wprintf(L"-6-Rechercher\n")     ;
   wprintf(L"-7-Modifier\n")       ;
   wprintf(L"-8-Supprimer\n")      ;
   wprintf(L"-0-Quitter\n")        ;
   wprintf(L"Choix : ")            ;
   wscanf(L"%d",&choix)            ;
   ...
   switch(choix)
   {
    ...
    case 0 : quitter()   ;
             break       ;
    default : wprintf(L"Erreur de saisie\n");
             break                        ;
   }
  }
}
...
void saisie()
{
  ...
  wcscpy(eleve.nom,L"\0");

  while ( wcscmp(eleve.nom,L"FIN") != 0  )
  {
    wprintf(L"nom (fin pour terminer) : ");
    wscanf(L"%ls",eleve.nom)              ;
    conv_maj_accentsW(eleve.nom,conserve_accents);

    if  ( wcscmp(eleve.nom,L"FIN") != 0  )
    {
     wprintf(L"Prénom : ");
     wscanf(L"%ls",eleve.prenom)  ;
     conv_maj_accentsW(eleve.prenom,conserve_accents)   ;
     wprintf(L"age : ")         ;
     wscanf(L"%d", &eleve.age)  ;
     tabeleves[i++] = eleve   ;
    }
  }
  nbeleves = i ;
  wprintf(L"%d élève(s) saisi(s)\n",nbeleves);
  a_sauvegarder=1;
}
/* --------------------------------- */
...
void affichage()
{
  ...
  if (nbeleves == 0)
    wprintf(L"Aucun élève à afficher\n");
  else
  {
   for (i=0; i<nbeleves;i++)
   {
     eleve = tabeleves[i];
     wprintf(L"%-20ls %-20ls %4d\n",eleve.nom, eleve.prenom, eleve.age);
   }
  }
}
/* --------------------------------- */
...
void sauvegarde()
{
  ...
  if (nbeleves == 0)
    wprintf(L"Aucun élève à sauvegarder\n");
  else
  {
   ...
   {
     eleve = tabeleves[i];
     fwprintf(f1,L"%ls %ls %d\n",eleve.nom, eleve.prenom, eleve.age);
   }
   fclose(f1);
   wprintf(L"%d élève(s) sauvegardé(s)\n",nbeleves);
  }
  a_sauvegarder=0;
}
/* --------------------------------- */
...
void chargement()
{
  ...
  wprintf(L"Nom du fichier à charger : ");
  wscanf(L"%s",nomfichier);
  ...
  while ( ! feof(f1)  )
  {
    lectureOK = fwscanf(f1,L"%ls %ls %d\n",eleve.nom, eleve.prenom, &eleve.age) ;
    if (lectureOK != EOF)
    {
     conv_maj_accentsW(eleve.nom,conserve_accents);
     conv_maj_accentsW(eleve.prenom,conserve_accents);
...
void supprime_tous_les_eleves()
{
 nbeleves      = 0 ;
 a_sauvegarder = 1 ;
 wprintf(L"Tous les élèves ont été supprimés\n");
}
/* ------------------------------- */
...
void  modification()
{
    ...
    if (nbeleves == 0)
        wprintf(L"Aucun élève actuellement pour la modification\n");
    else
    {
      wprintf(L"Entrez un nom de la personne à modifier : ");
      wscanf(L"%ls",nom);
      conv_maj_accentsW(nom,conserve_accents);
      numero = recherche(nom);
      if (numero == NON_TROUVE)
      {
         wprintf(L"Aucune modification : %s n'a pas été trouvé\n",nom);
      }
      else
      {
         eleve = tabeleves[numero];

         wprintf(L"nom actuel : %ls --",eleve.nom);
         wprintf(L"Nouveau nom : ");
         wscanf(L"%ls",eleve.nom);
         conv_maj_accentsW(eleve.nom,conserve_accents);

         wprintf(L"prénom actuel : %ls --",eleve.prenom);
         wprintf(L"Nouveau prénom : ");
         wscanf(L"%ls",eleve.prenom);
         conv_maj_accentsW(eleve.prenom,conserve_accents);

         wprintf(L"âge actuel : %d --",eleve.age);
         wprintf(L"Nouvel âge : ");
         wscanf(L"%d",&eleve.age);
...
void  suppression()
{
    ...
    if (nbeleves == 0)
        wprintf(L"Aucun élève actuellement pour la suppression\n");
    else
    {
      wprintf(L"Entrez le nom de la personne à supprimer : ");
      wscanf(L"%ls",nom);
      conv_maj_accentsW(nom,conserve_accents);
      numero = recherche(nom);
      if (numero == NON_TROUVE)
      {
         wprintf(L"Aucune suppression : %ls n'a pas été trouvé\n",nom);
      }
      else
      {
        eleve = tabeleves[numero];
        wprintf(L"Elève %ls %ls %d supprimé !\n",eleve.nom, eleve.prenom, eleve.age);
...
void verif_sauvegarde()
{
  wchar_t reponse[20], UnelettreWide=L'\0';

  if (a_sauvegarder)
  {
    wprintf(L"Vos données sont modifiées. Voulez-vous faire une sauvegarde ? (o/n) : ");
    wscanf(L"%ls",reponse);
    UnelettreWide = towupper(reponse[0]);
    if (UnelettreWide == L'O')
    {
      sauvegarde();
    }
  }
...
void rechercher_eleve()
{
   ...
   if (nbeleves == 0)
     wprintf(L"Aucun élève actuellement pour la recherche\n");
   else
   {
     wprintf(L"Entrez un nom à rechercher : ");
     wscanf(L"%ls",nom);
     conv_maj_accentsW(nom,conserve_accents);
     numero = recherche(nom);
     if (numero == NON_TROUVE)
     {
       wprintf(L"%ls n'a pas été trouvé\n",nom);
     }
     else
     {
        eleve = tabeleves[numero];
